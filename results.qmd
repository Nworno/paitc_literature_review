...
description_articles <- articles_level_df %>% 
  select(publication_year, country_first_author, country_last_author, methodologist_yn, 
         ...
         paic_count, paic_conducted, systematic_review_yn) %>% 
...
```{r}
tableby_to_flextable(description_articles, Yes, `Not mentioned`, Total) %>% 
  format_flextable() %>% 
...
ggsave(filename = file.path(article_figure_path, "publication_year_articles.png"), height = 15, width = 20, units = "cm")
...
comparison_level_df_dm %>% 
  filter(outcome_variable_type == "Binary (eg rates)", 
         contrast_effect_type == "Rates/ratio difference") 
...
ttt_level_df %>% 
  ...
  mutate(mab = factor(mab, levels = c(FALSE, TRUE), labels = c("No", "Yes"))) %>% 
  ...
  mutate(perc = count / sum(count) * 100) %>% {
    ggplot(data = ., aes(x = publication_year, y = count)) +
      ...
      geom_text(aes(fill = mab, label = paste0(format(perc, digits = 1, nsmall = 1), "%")),
                 position = position_stack(vjust = 0.5),
                 size = 3,
                 color = "white",
                 show.legend = FALSE) + 
...
ttt_df %>% 
  ...
  left_join(distinct(ttt_mapped[, c("treatment_name", "first", "second", "third", "car_t_cells")]),
            by = "treatment_name") %>%
  group_by(ipd_yn) %>% 
  count(first) %>% 
...
plot_layout <- outcomes_df %>%   
  ...
  filter(adjustment == "adjusted", 
         contrast_effect_type == "Rates/ratio") %>% 
...
plot_layout +
  ...
  annotate("text", label = "\n\nFavors aggregated data", x = 13, y = 8, size = 5) +
  geom_segment(aes(x = 1.2, xend = 90, y = 2.75, yend = 2.75),
               size = 0.7,
               arrow = arrow(length = unit(0.2,"cm")),
               inherit.aes = FALSE) +
  annotate("text", label = "\n\nFavors IPD", x = 0.15, y = 8, size = 5) +
  geom_segment(aes(x = 0.82, xend = 0.011, y = 2.75, yend = 2.75),
               size = 0.7,
               arrow = arrow(length = unit(0.2,"cm")),
               inherit.aes = FALSE) +
...
ggsave("C:/Users/aserret-larmande/Documents/these/research/articles_redaction/paic_methodological_review/figures/ratio_treatment_effect.png", width = 20, height = 10, unit = "cm")
```
