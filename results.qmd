...
ties_w_pharma_df <- general_information_df %>% 
  filter(questions %in% c("Ties with pharmaceutical industry mentionned in competing interest/conflict of interest/disclaimer, or any equivalent section in the article", 
  ...
  mutate(ties_pharma = ifelse(!grepl("Pharmaceutical", `Positions of study investigators (for any authors of the article, any that applies)`) & 
                                `Ties with pharmaceutical industry mentionned in competing interest/conflict of interest/disclaimer, or any equivalent section in the article` != "Yes", FALSE, TRUE)) 
...
general_information_df %>% 
  ...
  coord_flip() + 
  labs(x = NULL,
       title = "Ties with pharmaceutical industry", 
       y = "Count")
```
...
same_comparisons %>% write_tsv("data/extraction/adjudicated/similar_comparisons.tsv")
...
wide_results_outcome %>% 
  ...
  tableby(benefit_ipd ~ significant, strata = ties_pharma, data = .) %>% 
...
outcomes_df %>% 
  ...
  # geom_text(data = labelling_data,
  #           aes(label = left_side, x = (effect_cutoff + left_x) / 2, y = max_num_comparison *1.2),
  #           inherit.aes = FALSE, 
  #           size = 5) +
  # geom_text(data = labelling_data,
  #           aes(label = right_side, x = (effect_cutoff + right_x) / 2, y = max_num_comparison *1.2),
  #           inherit.aes = FALSE, 
  #           size = 5) +
  # annotate("text", -Inf, Inf, label = "Favors IPD", hjust = -1, vjust = 2, size = 5) +
  # annotate("text", -Inf, Inf, label = "Favors non-IPD", hjust = -2.5, vjust = 2, size = 5) +
