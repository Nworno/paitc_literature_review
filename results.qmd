...
```{r}
summary(description_articles_df)
```
...
```{r}
library(flextable)
description_articles_df <- as_tibble(description_articles)

description_articles %>%
  summary() %>% 
  flextable()

str(description_articles)

label_total <- glue::glue("Total (n = {nrow(articles_level_df)})")

flextable_description_articles <- description_articles_df %>% 
  select(variable = term, modalities = label, last_col()) %>% 
  mutate(variable = if_else(Total == "", modalities, NA_character_), 
         modalities = if_else(Total != "", modalities, NA_character_)
         ) %>% 
  mutate(across(c(Total), .fns = separate_elements), 
         variable = replace_in_vec(variable, swap_names_values(renaming_questions_list))) %>% 
  flextable()

indic_hlines <- (which(!is.na(flextable_description_articles$body$dataset$variable)) - 1)
indic_hlines <- indic_hlines[2: length(indic_hlines)]
flextable_description_articles %>% 
  # set_header_labels(values = c("label" = "test1", "Overall" = "test2", "Total" = "test3")) %>%
  set_header_labels(values = c("variable" = "",
                               "modalities" = "",
                               "Total" = label_total)) %>%
  add_header_row(values = c("", "n (%)"), colwidths = c(2, 1), top = FALSE ) %>% 
  flextable::align(i = 1, j = 1:2, align = "center", part = "header") %>% 
  flextable::theme_booktabs() %>% 
  hline(i = indic_hlines, part = "body", border = fp_border(width = 2)) %>% 
  autofit()
...
```{r}
# kableExtra can only save to html/png/pdf
# 
# library(kableExtra)
# description_articles %>% 
#   as_tibble() %>% 
#   select(label, Overall, Total) %>% 
#   kableExtra::kbl() %>% 
#   kable_minimal() %>% 
...
#   row_spec(c(3, 10, 15), underline = FALSE, bold = TRUE, align = 'left') %>%
#   column_spec(1, background = 'FDEBD0') %>% 
#   pack_rows('Demographics', 1, 20, indent = FALSE, label_row_css = "text-align: left; font-style: italic") %>%
# pack_rows('COVID-19 Symptoms', 20, 22, indent = FALSE, label_row_css = "text-align: left; font-style: italic")
...
# TODO: retravailler ça en fonction de la lecture de la problématique des 
# échelles d'outcomes de Phillippo
renaming_contrast <- c(
  ...
  "HR" = "Rates/ratio", 
  "Means difference" = "Means/median difference", 
  "Median difference" = "Means/median difference", 
  "RMST difference at 12 months" = "Means/median difference",
  "Risk difference" = "Rates/ratio difference", 
  "Rate difference" = "Rates/ratio difference", 
  "Proportion difference" = "Rates/ratio difference",
  "Proportions difference" = "Rates/ratio difference"
)
...
adjustment_level_df <- bind_rows(left_side_outcome, right_side_outcome) %>%
...
main_methods <- comparison_level_df_dm %>%
  ...
  select(systematic_review_yn, primary_outcome_yn, ss_ipd_yn, ss_nonipd_yn, ss_adjusted_yn) %>% 
  tableby( ~ ., data = .) 
```
...
```{r}
tableby2word(secondary_methods, "output_tables/secondary_methods.docx")
```
...
```{r}
tableby2word(adjustment_variables, "output_tables/adjustment_variables.docx")
```
...
results_tableby <- comparison_level_df_dm %>% 
  select(unadjusted_yn, adjusted_yn, reduction_ess_percent, weights_yn, 
         reduction_ess_percent, perc_change_effect) %>% 
  tableby( ~ ., data = ., 
           control = tableby.control(numeric.stats = c("median", "q1q3","range", "meansd")))
summary(results_tableby)
```

```{r}
tableby2word(results_tableby, "output_tables/results_tableby.docx")
```
...
adjustment_level_tableby <- adjustment_level_df %>% 
  select(adjustment, significant, benefit_ipd) %>% 
  tableby(adjustment ~ benefit_ipd, data = ., strata = significant)
```
...
```{r}
tableby2word(adjustment_level_tableby, "output_tables/adjustement_level.docx")
```
