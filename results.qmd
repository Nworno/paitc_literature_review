...
```{r Direction for improvement}
# Manually writing which direction for the treatment contrast effect to make it benefitial
left_join(wide_methodo, wide_results) %>%
  select(primary_outcome_name, 
         primary_outcome_type, 
         contrast) %>%
  distinct() %>% 
  write_tsv("data/extraction/adjudicated/outcome_benefits_raw.tsv")
...
renaming_outcomes <- c(
  ...
  "HR" = "HR", 
  "Proportion difference" = "Proportions difference"
)
...
#TODO: Many NA for the treatment contrast effect --> to be investigated

wide_results_outcome <- wide_results %>% 
   mutate(contrast = replace_in_vec(contrast, renaming_outcomes)) %>% 
  select(doi, n_itc, contrast, effect_direction, unadjusted_lb_ci, unadjusted_ub_ci, adjusted_lb_ci,
         adjusted_ub_ci, unadjusted_significant, adjusted_significant,
         unadjusted_ci_cutoff, adjusted_ci_cutoff,
         unadjusted_num_pval, adjusted_num_pval, 
         unadjusted_effect, adjusted_effect) %>% 
  pivot_longer(names_pattern = "((?:un)?adjusted)(_)(.*)", 
               cols = !c(doi, n_itc, contrast, effect_direction), 
               names_to = c("adjustment", NA, "indicator"), 
               values_to = "values") %>%
  pivot_wider(id_cols = c(doi, n_itc, contrast, effect_direction, adjustment), 
              names_from = indicator, 
              values_from = values) %>% 
  mutate(across(c(lb_ci, ub_ci, num_pval, effect, ci_cutoff), as.numeric)) 
...
right_side_outcome <- wide_results_outcome %>% 
  ...
  mutate(across(c(lb_ci, ub_ci, effect),
                .fns = function(x) {
                  ifelse(ci_cutoff == 0, -x, 
                         ifelse(ci_cutoff == 1, 1/x, NA))
                })
  ) %>% 
...
wide_results_outcome %>% 
  #TODO: fix contrast == NA
  filter(!is.na(contrast)) %>%
  ggplot(aes(x = adjustment, y = effect, color = significant)) +
  geom_jitter(width = 0.1) +
  geom_hline(aes(yintercept = ci_cutoff), linetype = "dashed", color = "black") + 
...
long_results_outcome %>% 
  filter(indicator == "effect") %>%
  ggplot(aes(x = adjustment, y = values)) +
  geom_jitter(width = 0.1) +
  facet_wrap(~ contrast, scales = "free_y")
